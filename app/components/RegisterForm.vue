...
>
    <div class="w-full">
      <div class="mb-6">
        ...
        <h2 class="text-3xl font-bold mb-8 text-[#0F0F0F] dark:text-[#F3F4F6]">Créer un compte</h2>
      </div>

      <CUButton
        loading-auto
        @click="signInWithGithub"
        label="Continuez avec Github"
      ...
      <p class="text-center text-[#0F0F0F]/70 dark:text-[#F3F4F6]/70 text-sm">
        J'ai déjà un compte  
        <NuxtLink to="/auth/login" class="text-text hover:underline underline underline-offset-4">Se connecter</NuxtLink>
      </p>
    </div>
</UForm>
<VerifieEmail @close="()=>{emits('closeSignup')}" :email="state.email" v-else />
</template>
